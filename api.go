...
func (a *API) Do(ctx context.Context, method, path string, body any, params url.Values) APIResponse {
	...
	if err != nil {
		return misconfigured(fmt.Errorf("%w: body could not be encoded: %v", ErrMisconfigured, err))
	}
	...
	if err != nil {
		...
		return APIResponse{Error: "auth: " + err.Error(), Retryable: true, Err: err}
	}
	...
	if err != nil {
		...
		return APIResponse{Error: err.Error(), Retryable: true, Err: err}
	}
...
func (a *API) perform(ctx context.Context, method string, target *url.URL, bearer string, payload []byte) (*http.Response, []byte, error) {
	...
	default:
		return nil, nil, fmt.Errorf("%w: unsupported api method %q", ErrMisconfigured, method)
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf("%w: %v", ErrMisconfigured, err)
	}
...
func (a *API) buildURL(path string, params url.Values) (*url.URL, error) {
	if a.baseURL == "" {
		return nil, fmt.Errorf("%w: APIBaseURL is not configured", ErrMisconfigured)
	}
	if a.apiKey == "" {
		return nil, fmt.Errorf("%w: APIKey is not configured", ErrMisconfigured)
	}
	if path == "" {
		return nil, fmt.Errorf("%w: api path is required", ErrMisconfigured)
	}
	base, err := url.Parse(strings.TrimSuffix(a.baseURL, "/"))
	if err != nil {
		return nil, fmt.Errorf("%w: APIBaseURL is not a valid URL", ErrMisconfigured)
	}

	target, err := url.Parse(path)
	if err != nil {
		return nil, fmt.Errorf("%w: api path is not a valid URL", ErrMisconfigured)
	}
	if target.IsAbs() {
		if target.Scheme != base.Scheme || target.Host != base.Host {
			return nil, fmt.Errorf("%w: api path %q is not on APIBaseURL (%s)", ErrMisconfigured, path, a.baseURL)
		}
	} else {
...
func misconfigured(err error) APIResponse {
	return APIResponse{Error: err.Error(), Err: err}
}
